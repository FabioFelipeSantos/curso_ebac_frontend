...
function calculatingArea(base: number, height: number) {
	return base * height;
}

// Arrow function follow the same TYPE's rules that the usual function.
const calculatingTriangleArea = (base: number, height: number): number => (base * height) / 2;

function toSum(...numbers: number[]) {
...
function toShow(num: number): string | boolean | number {
