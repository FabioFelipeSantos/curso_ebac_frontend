...
namespace casting {
	...
	(isRaining as number).toFixed(); // Using a method only for numbers
	(isRaining as string).toLowerCase(); // Using a method only for strings
	(isRaining as string[]).forEach((x) => console.log(x)); // Using a method only for arrays
	...
	let age: string = 36 as unknown as string;
