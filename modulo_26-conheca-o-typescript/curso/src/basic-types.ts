...
const myName: string = "Fabio Santos";
...
const roommates: string[] = ["Maria", "Gascoigne", "Ludwig", "Laurence", "Amelia", "Micolash"];
const technologies: Array<string> = ["js", "sass", "jsx"];
...
const grades: ReadonlyArray<number> = [7, 9, 6, 5];

// The TUPLA type allow you set different types to an array.
const student_1: [name: string, isStudying: boolean, grade: number, age: number] = ["Fabio", true, 3, 36];

// The UNION operator ( "|" - pipe symbol) allow we set more than one type for a variable.
let isOpen: boolean | number = false;
isOpen = 1;
